...
RededgeM_processing_dp <- function(dir_CRP, dir_images, metadata_trsf = T) {
...
for (i in 1:5) { # i = n? of bands = n? of RtR factors
  foreach (j = seq(i,length(image_corr_list), 5), .packages = c("raster")) %dopar% { # j = sequential n? of the picture in the list, 5 = n? of bands
...
if(metadata_trsf == T) {
  ...
  pb <- txtProgressBar(min = 0, max = length(image_list), style = 3)
  foreach (i = 1:length(image_list)) %dopar% {
    exiftool_call(args = c("-all=", "-TagsFromFile", image_list[i], "-all:all", "-xmp", image_ref_list[i]),
                  quiet = T)
    setTxtProgressBar(pb, i)
  }
